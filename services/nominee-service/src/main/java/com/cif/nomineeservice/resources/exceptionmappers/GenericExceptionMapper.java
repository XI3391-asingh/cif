package com.cif.nomineeservice.resources.exceptionmappers;
...
public class GenericExceptionMapper implements ExceptionMapper<RuntimeException> {
    ...
    public GenericExceptionMapper(MetricRegistry metricRegistry) {
    ...
    public Response toResponse(RuntimeException e) {
        ...
        if (e instanceof RecordNotFoundException recordNotFoundException) {
            response = Response.status(RecordPersistException.HTTP_ERROR_CODE.getStatusCode()).entity(
                    ResponseHelper.buildResponseApi(UUID.randomUUID().toString(), RecordNotFoundException.HTTP_ERROR_CODE.getReasonPhrase(), e.getMessage())).build();
        } else if (e instanceof RecordPersistException recordPersistException) {
            response = Response.status(RecordPersistException.HTTP_ERROR_CODE.getStatusCode()).entity(
                    ResponseHelper.buildResponseApi(UUID.randomUUID().toString(), RecordPersistException.HTTP_ERROR_CODE.getReasonPhrase(), e.getMessage())).build();
        } else {
            response = Response.status(HTTP_ERROR_CODE.getStatusCode()).entity(
                    ResponseHelper.buildResponseApi(UUID.randomUUID().toString(), HTTP_ERROR_CODE.getReasonPhrase(), TECHNICAL_ERROR)).build();
        }
        exceptions.mark();
        return response;
    }
